    ...
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        <h2 className="text-xs font-bold tracking-[0.2em] uppercase mb-8 text-gray-500 flex items-center gap-4">
          <span className="w-8 h-[1px] bg-gray-400"></span> Experience
        </h2>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-[#f5f5f5] flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-200 border border-neutral-300 p-2" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-display font-bold text-neutral-400 uppercase tracking-tight">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-display font-bold text-neutral-400 uppercase tracking-tight">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
